# 🍕 Pizza Sales Data Analysis

## 📌 Project Overview
**Role:** Data Analyst
**Tools:** MySQL, Google Sheets, Google Docs
**Objective:** Analyzed 1 year of sales data (20,000+ records) to identify peak operational hours, best-selling items, and revenue growth opportunities.

## 📂 Project Files
* **[SQL Analysis Code](Pizza%20Sales%20Analysis%20SQL%20Queries.sql):** The complete script containing all 13 queries solved.
* **[Final Business Report](Pizza%20Sales%20Project%20Report..pdf):** Executive summary of insights and recommendations.

## 📊 Key Findings
* **Busiest Days:** Orders peak on Friday evenings (6-8 PM) and Saturday afternoons (12-2 PM).
* **Top Revenue Item:** The "Thai Chicken Pizza" generates the highest revenue.
* **Customer Behavior:** Large pizzas account for 45% of total sales, suggesting families/groups are the primary customer base.

## 📈 Dashboard
**[Click Here to View Dashboard PDF](Pizza%20Sales%20Dashboard%20-%20Dashboard%20(1).pdf)**
*(Visual breakdown of sales trends and category performance)*
